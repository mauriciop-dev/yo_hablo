...
app.post('/api/tts/gemini', async (req, res) => {
  try {
    ...
    if (!response.ok) throw new Error(`Gemini TTS error ${response.status}: ${await response.text()}`);
    const data = await response.json();
...
app.post('/api/stt/groq', upload.single('audio'), async (req, res) => {
  try {
    ...
    const langHint = (req.body?.language || '').split('-')[0];
    if (langHint) groqForm.append('language', langHint);
